# Build entry points. Every target here is what CI runs, so a green CI run and a local
# run are the same commands.
#
...
# D8: build number is the CI run number, falling back to 1 for local builds.
